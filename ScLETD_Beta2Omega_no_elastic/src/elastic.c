...
elastic_malloc()
{
  ...
  for (p = 0; p < NELASTIC; p++)
  {
  	...
  	tmpy_fftre[p] = (double *)_mm_malloc(sizeof(double) * offset3, 256);
  	tmpy_fftim[p] = (double *)_mm_malloc(sizeof(double) * offset3, 256);
  }
  tmpy_re = (double *)_mm_malloc(sizeof(double) * offset3, 256);
  ...
  for (p = 0; p < NELASTIC; p++)
  {
 	for (q = 0; q < NELASTIC; q++)
	{
  	...
  	hipMalloc (&tmpy_fftRE[p], offset3*sizeof(double));
  	hipMalloc (&tmpy_fftIM[p], offset3*sizeof(double));
  }
  hipMalloc (&tmpy_RE1, offset3*sizeof(double));
