...
define_mpi_type (void)
{
  ...
  MPI_Type_contiguous (Approx * (ny - 2 * nghost + Approx * 2) * (nz - 2 * nghost + Approx * 2), MPI_DOUBLE, &conv_left_right);
  MPI_Type_contiguous ((nx - 2 * nghost) * Approx * (nz - 2 * nghost + Approx * 2), MPI_DOUBLE, &conv_top_bottom);
  MPI_Type_contiguous ((nx - 2 * nghost) * (ny - 2 * nghost) * Approx, MPI_DOUBLE, &conv_front_back);
...
sw_cart_creat (void)
{
  ...
  for (n = 0; n < nac; n++) {
    ...
    MPI_Send_init (&ac[n].fieldEs_left[0], 1, left_right, left, 9, MPI_COMM_WORLD, &ac[n].ireq_left_right_felas[0]);
    MPI_Recv_init (&ac[n].fieldEr_right[0], 1, left_right, right, 9, MPI_COMM_WORLD, &ac[n].ireq_left_right_felas[1]);
    MPI_Send_init (&ac[n].fieldEs_right[0], 1, left_right, right, 9, MPI_COMM_WORLD, &ac[n].ireq_left_right_felas[2]);
    MPI_Recv_init (&ac[n].fieldEr_left[0], 1, left_right, left, 9, MPI_COMM_WORLD, &ac[n].ireq_left_right_felas[3]);
    MPI_Send_init (&ac[n].fieldEs_top[0], 1, top_bottom, top, 9, MPI_COMM_WORLD, &ac[n].ireq_top_bottom_felas[0]);
    MPI_Recv_init (&ac[n].fieldEr_bottom[0], 1, top_bottom, bottom, 9, MPI_COMM_WORLD, &ac[n].ireq_top_bottom_felas[1]);
    MPI_Send_init (&ac[n].fieldEs_bottom[0], 1, top_bottom, bottom, 9, MPI_COMM_WORLD, &ac[n].ireq_top_bottom_felas[2]);
    MPI_Recv_init (&ac[n].fieldEr_top[0], 1, top_bottom, top, 9, MPI_COMM_WORLD, &ac[n].ireq_top_bottom_felas[3]);
    MPI_Send_init (&ac[n].felas[nx * ny * nghost], 1, front_back, front, 9, MPI_COMM_WORLD, &ac[n].ireq_front_back_felas[0]);
    MPI_Recv_init (&ac[n].fieldEr_back[0], 1, front_back, back, 9, MPI_COMM_WORLD, &ac[n].ireq_front_back_felas[1]);
    MPI_Send_init (&ac[n].felas[nx * ny * (nz - nghost - (nghost + 2))], 1, front_back, back, 9, MPI_COMM_WORLD, &ac[n].ireq_front_back_felas[2]);
    MPI_Recv_init (&ac[n].fieldEr_front[0], 1, front_back, front, 9, MPI_COMM_WORLD, &ac[n].ireq_front_back_felas[3]);
  ...
  for (n = 0; n < nch; n++) {
    ...
    MPI_Send_init (&ch[n].fieldCIs_left[0], 1, left_right, left, 9, MPI_COMM_WORLD, &ch[n].ireq_left_right_felas[0]);
    MPI_Recv_init (&ch[n].fieldCIr_right[0], 1, left_right, right, 9, MPI_COMM_WORLD, &ch[n].ireq_left_right_felas[1]);
    MPI_Send_init (&ch[n].fieldCIs_right[0], 1, left_right, right, 9, MPI_COMM_WORLD, &ch[n].ireq_left_right_felas[2]);
    MPI_Recv_init (&ch[n].fieldCIr_left[0], 1, left_right, left, 9, MPI_COMM_WORLD, &ch[n].ireq_left_right_felas[3]);
    MPI_Send_init (&ch[n].fieldCIs_top[0], 1, top_bottom, top, 9, MPI_COMM_WORLD, &ch[n].ireq_top_bottom_felas[0]);
    MPI_Recv_init (&ch[n].fieldCIr_bottom[0], 1, top_bottom, bottom, 9, MPI_COMM_WORLD, &ch[n].ireq_top_bottom_felas[1]);
    MPI_Send_init (&ch[n].fieldCIs_bottom[0], 1, top_bottom, bottom, 9, MPI_COMM_WORLD, &ch[n].ireq_top_bottom_felas[2]);
    MPI_Recv_init (&ch[n].fieldCIr_top[0], 1, top_bottom, top, 9, MPI_COMM_WORLD, &ch[n].ireq_top_bottom_felas[3]);
    MPI_Send_init (&ch[n].felas[nx * ny * nghost], 1, front_back, front, 9, MPI_COMM_WORLD, &ch[n].ireq_front_back_felas[0]);
    MPI_Recv_init (&ch[n].fieldCIr_back[0], 1, front_back, back, 9, MPI_COMM_WORLD, &ch[n].ireq_front_back_felas[1]);
    MPI_Send_init (&ch[n].felas[nx * ny * (nz - nghost - (nghost + 2))], 1, front_back, back, 9, MPI_COMM_WORLD, &ch[n].ireq_front_back_felas[2]);
    MPI_Recv_init (&ch[n].fieldCIr_front[0], 1, front_back, front, 9, MPI_COMM_WORLD, &ch[n].ireq_front_back_felas[3]);
#endif
  }
}
